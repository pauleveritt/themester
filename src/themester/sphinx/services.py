...
class PathTo:
    ...
    def __call__(
        ...
        page_context = self.container.get(PageContext)
        ...
        if baseuri is None:
            ...
            baseuri = target_uri(page_context.pagename).rsplit("#", 1)[0]
...
def svcs_setup(registry: Registry) -> None:
    registry.register_factory(BuilderConfig, BuilderConfig)
    registry.register_factory(PathTo, PathTo)
    registry.register_factory(RelativeUri, RelativeUri)
    registry.register_factory(TargetUri, TargetUri)
