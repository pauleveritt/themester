...
def test_normalize(current: PurePosixPath | str, expected: PurePosixPath) -> None:
    """Test passing resources, paths, and strings."""
    result = normalize(current)
    assert result.parts == expected.parts
...
def test_relative_mixture(
    ...
    path = relative(current, target, static_prefix=static_prefix, suffix=suffix)
